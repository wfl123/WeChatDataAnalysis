...
def copy_wechat_data(
    ...
    total_mb = total_bytes / 1024 / 1024
    log_info(f"共 {total_count} 个文件，总大小约 {total_mb:.1f} MB")
    ...
    small_files = [(s, z) for s, z in all_files if z < SMALL_FILE_THRESHOLD]
    large_files = [(s, z) for s, z in all_files if z >= SMALL_FILE_THRESHOLD]
    ...
    # ── 小文件：多线程并发 ──────────────────────────────────────────────────
    if small_files:
        log_info(f"小文件 {len(small_files)} 个，并发线程数 {workers}")

        def _task(item):
            src, size = item
            dest = _dest_path(src)
            _copy_one(src, dest, size, stats)
    ...
    # ── 大文件：顺序拷贝 ────────────────────────────────────────────────────
    if large_files:
        log_info(f"大文件 {len(large_files)} 个，顺序拷贝")
