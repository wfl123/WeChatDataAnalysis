...
set PYARMOR=D:\APPLOAD\Anaconda\envs\modelscope_py39\Scripts\pyarmor.exe
...
echo [2/3] 混淆源码（pyarmor 8.x）...
mkdir dist_obf

REM pyarmor 8.x 用直接命令，不用 -m
"%PYARMOR%" gen --output dist_obf 01_wx_win.py wx_find_and_copy.py

if %errorlevel% neq 0 (
    echo [WARN] pyarmor 混淆失败，使用原始源码打包
